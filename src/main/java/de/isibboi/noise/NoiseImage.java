...
public class NoiseImage {
	...
	public NoiseImage(Function f, int scale, int noiseScale) throws Exception {
		noiseImage = new BufferedImage(IMAGE_WIDTH * scale, IMAGE_HEIGHT
				* scale, BufferedImage.TYPE_INT_RGB);

		JFrame noiseFrame = new JFrame("Noise");
		noiseFrame.getContentPane().setPreferredSize(
				new Dimension(Math.min(noiseImage.getWidth(), 800), Math.min(
						noiseImage.getHeight(), 800)));
		...
		c = new PerspectiveCamera(cameraPosition,
				cameraPosition.multiplyScalar(-1), new Vector(-0.5, 2.5,
						-Math.sqrt(2)), cameraPosition.multiplyScalar(2.8));
		...
		for (int x = 0; x < noiseImage.getWidth(); x++) {
			for (int y = 0; y < noiseImage.getHeight(); y++) {
				noiseImage.setRGB(
						x,
						y,
						noiseFunction((double) x / noiseImage.getWidth()
								* realNoiseWidth - (realNoiseWidth / 2),
								(double) y / noiseImage.getHeight()
										* realNoiseHeight
										- (realNoiseHeight / 2)));
		...
		System.out.println("Rendering took " + (System.currentTimeMillis() - renderStartTime) + "ms");
		...
		noiseFrame
				...
				.drawImage(
						...
						0,
						0,
						(int) noiseFrame.getContentPane().getPreferredSize()
								.getWidth(),
						(int) noiseFrame.getContentPane().getPreferredSize()
								.getHeight(), null);
	...
	private int noiseFunction(final double x, final double y) throws Exception {
		...
		double shade = Math.max(
				new Vector(-1, -1, 1).getNormalized().dot(
						f.normal(hit.getX(), hit.getY())), 0);
		...
		c = new Color((int) (red * shade), (int) (green * shade),
				(int) (blue * shade));
